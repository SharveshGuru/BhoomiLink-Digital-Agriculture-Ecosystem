...
export default function EditRawMaterial({ material, onUpdate, onCancel }) {
  const [formData, setFormData] = useState({
    name: material.name || "",
    category: material.category || "fertilizers",
  ...
  useEffect(() => {
    if (material.imageUrl) {
      setImagePreview(material.imageUrl); // Assuming backend returns image URL
    }
  }, [material]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.put(`/rawmaterials/${material.id}`, formDataToSend, {
        headers: {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold text-green-800">Edit Raw Material</h2>
